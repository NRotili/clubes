...
@section('title', 'Editar — ' . $profesor->nombreCompleto())
...
<div class="mb-6 flex items-center gap-2 text-sm text-slate-500">
    ...
    <a href="{{ route('profesores.show', $profesor) }}" class="hover:text-slate-700 transition-colors">{{ $profesor->nombreCompleto() }}</a>
    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5"/>
    </svg>
...
<div class="mb-6">
    <h1 class="text-2xl font-bold text-slate-900">Editar profesor</h1>
</div>
...
<form method="POST" action="{{ route('profesores.update', $profesor) }}" novalidate>
    @csrf @method('PUT')
