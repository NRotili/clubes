...
@section('title', 'Editar — ' . $socio->nombreCompleto())
...
<div class="mb-6 flex items-center gap-2 text-sm text-slate-500">
    ...
    <a href="{{ route('socios.show', $socio) }}" class="hover:text-slate-700 transition-colors">{{ $socio->nombreCompleto() }}</a>
    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5"/>
    </svg>
...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        ...
        <p class="text-sm text-slate-500 mt-0.5">N° {{ $socio->numero_socio }} — {{ $socio->nombreCompleto() }}</p>
    </div>
</div>
...
<form method="POST" action="{{ route('socios.update', $socio) }}" enctype="multipart/form-data" novalidate>
    @csrf
    @method('PUT')
    ...
    <div class="flex items-center justify-between pt-2">
        {{-- Eliminar (fuera del form principal usando action="…") --}}
        <button type="button"
...
{{-- Form de eliminación separado para evitar forms anidados --}}
<form id="form-eliminar" method="POST" action="{{ route('socios.destroy', $socio) }}"
    onsubmit="return confirm('¿Eliminar al socio {{ addslashes($socio->nombreCompleto()) }}? Esta acción no se puede deshacer.')">
    @csrf
    @method('DELETE')
</form>
